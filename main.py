...
X_train_normalized = [(X_train[c] - feat_min) / (feat_max - feat_min) for c in np.arange(len(glob.classes()))]  # normalized matrix
...
y_test = [np.ones(X_train[0].shape[0],) * i for i in np.arange(3)]  # dictionaries
