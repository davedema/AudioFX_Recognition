...
def test():
    ...
    dict_test_features = testloop.getdicttestfeatures()  # test features
    X_test = [dict_test_features[c] for c in glob.classes()]
    columns_selected = dataloader.colums_selected()  # positions of selected features
    X_test_selected = [X_test[i][:, columns_selected] for i in np.arange(len(glob.classes()))]  # selection
    y_test = [np.ones(X_test[i].shape[0], ) * i for i in np.arange(3)]  # keys
    ...
    X_train_normalized_loaded = [
        ...
        for c in glob.classes()]  # train features
    X_train_normalized_loaded_selected = [
        X_train_normalized_loaded[i][:, columns_selected] for i in np.arange(len(glob.classes()))]  # selection
